...
if __name__ == "__main__":
    ...
    if response.status_code == 200:
        ...
        message = (
            "Currency: {:0.2f}/{:0.2f} USD, {:0.2f}/{:0.2f} EUR, {:0.2f}/{:0.2f} RUR"
        )
        logger.info(
            message.format(usd_buy, usd_sell, eur_buy, eur_sell, rur_buy, rur_sell)
        )
    else:
        logger.error("Currency worker error in response: {}", response.status_code)
